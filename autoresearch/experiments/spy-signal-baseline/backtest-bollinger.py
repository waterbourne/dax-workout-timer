#!/usr/bin/env python3
"""
SPY Options Signal Backtester v2.4 - Bollinger Bands %B
========================================================
Adds Bollinger Bands %B as a factor:
- Calls: %B < 0.1 (price at/near lower band, oversold)
- Puts: %B > 0.9 (price at/near upper band, overbought)
...
def calc_bollinger_bands(close, period=20, std_dev=2):
    ...
    return pct_b, upper_band, lower_band, sma
...
def compute_signals_for_day(spy, vix, idx):
    ...
    # Bollinger Bands %B
    pct_b = float(calc_bollinger_bands(spy_close_series, 20, 2)[0].iloc[-1])
    
    # --- CALL FACTORS (8 total with BB) ---
    call_factors = {
        ...
        'bb_low': pct_b < 0.1,  # Price near lower band
    }
    call_score = sum(call_factors.values())
    
    # --- PUT FACTORS (8 total with BB) ---
    put_factors = {
        ...
        'bb_high': pct_b > 0.9,  # Price near upper band
    }
...
def analyze_results(trades):
    print("\n" + "=" * 70)
    print("BACKTEST RESULTS - WITH BOLLINGER BANDS %B")
    print("=" * 70)
    ...
    for direction in ['CALL', 'PUT']:
        ...
        for score in [3, 4, 5, 6, 7, 8]:
            ...
            print(f"  Score {score}/8: {len(score_trades)} trades, {win_rate:.1f}% win, {avg_return*100:+.1f}% avg")
...
if __name__ == "__main__":
    print("SPY Options Signal Backtest - Bollinger Bands %B Added")
    print("=" * 70)
