#!/usr/bin/env python3
"""
SPY Options Signal Backtester v2.6 - Regime-Based Puts
========================================================
Only trade puts when market is in bearish regime (200 SMA falling).
Calls are traded in all market conditions.

Hypothesis: Puts fail because they trigger during bull markets.
Only trading puts during bear trends should improve win rate.
"""
...
def run_backtest():
    ...
    print("Calls: 4+ of 7 (all regimes)")
    print("Puts: 4+ of 7 (BEARISH REGIME ONLY)")
    print("=" * 70)
    ...
    for i in range(warmup, len(spy)):
        ...
        if signals['call_score'] >= 4:
            ...
            trades['CALL'].append({
                ...
                'is_bearish': signals['is_bearish'],
            })
        
        # Puts at 4+ (ONLY in bearish regime)
        if signals['is_bearish'] and signals['put_score'] >= 4:
            ret, exit_date, reason, days = simulate_trade(
...
def analyze_results(trades):
    print("\n" + "=" * 70)
    print("BACKTEST RESULTS - REGIME-BASED PUTS")
    print("Puts ONLY in bearish trends (200 SMA falling)")
    print("=" * 70)
...
if __name__ == "__main__":
    print("SPY Options Signal Backtest - Regime-Based Puts")
    print("=" * 70)
