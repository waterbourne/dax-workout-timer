#!/usr/bin/env python3
"""
SPY Options Signal System v2.2 (Consecutive Days Update)
=========================================================
Added consecutive days factor:
- Calls: 3+ down days suggests bounce
- Puts: 3+ up days suggests pullback

CALLS: 4+ of 7 factors (oversold bounce)
PUTS:  4+ of 7 factors (overbought fade)

Both require 4+ for alert.
"""
...
def analyze():
    ...
    call_factors = {
        ...
        'consecutive_down': {
            ...
            'label': f"3+ down days {'✅' if down_days >= 3 else ''} ({down_days} days)",
        },
    }
    ...
    put_factors = {
        ...
        'consecutive_up': {
            'triggered': up_days >= 3,
            'label': f"3+ up days {'✅' if up_days >= 3 else ''} ({up_days} days)",
        },
    }
