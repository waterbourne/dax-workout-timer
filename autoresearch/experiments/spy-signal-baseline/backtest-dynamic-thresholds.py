#!/usr/bin/env python3
"""
SPY Options Signal Backtester v2.3 - Dynamic Thresholds
========================================================
Tests dynamic thresholds based on 200 SMA slope:
- In bullish trend (200 SMA rising): Put threshold = 5 (harder to trigger)
- In bearish trend (200 SMA falling): Put threshold = 4 (normal)

Hypothesis: Puts fire too often in bull markets, hurting win rate.
Raising threshold to 5+ in bull markets should improve put win rate.
"""
...
def compute_signals_for_day(spy, vix, idx):
    ...
    is_bullish = sma200_slope > 0
    call_threshold = 4  # Always 4 for calls
    put_threshold = 5 if is_bullish else 4  # 5 in bull, 4 in bear
...
def run_backtest():
    ...
    for i in range(warmup, len(spy)):
        ...
        # Check for call signal (threshold = 4)
        if signals['call_score'] >= signals['call_threshold']:
            ...
            trades['CALL'].append({
                ...
                'threshold': signals['call_threshold'],
                'entry_price': signals['spy_close'],
                'return': ret,
                'exit_reason': reason,
                'days_held': days,
                'sma200_slope': signals['sma200_slope'],
                'is_bullish': signals['is_bullish'],
            })
        
        # Check for put signal (dynamic threshold)
        if signals['put_score'] >= signals['put_threshold']:
            ret, exit_date, reason, days = simulate_trade(
            ...
            trades['PUT'].append({
                ...
                'threshold': signals['put_threshold'],
                'entry_price': signals['spy_close'],
                'return': ret,
                'exit_reason': reason,
                'days_held': days,
...
def analyze_results(trades):
    print("\n" + "=" * 70)
    print("BACKTEST RESULTS - DYNAMIC THRESHOLDS")
    print("Put threshold = 5 in bullish trend, 4 in bearish trend")
    print("=" * 70)
    ...
    for direction in ['CALL', 'PUT']:
        ...
        # By regime for puts
        if direction == 'PUT':
            bull_trades = [t for t in all_trades if t['is_bullish']]
            bear_trades = [t for t in all_trades if not t['is_bullish']]
            
            print(f"\n  BY REGIME (Dynamic Thresholds):")
            if bull_trades:
                ...
                avg_score = sum(t['score'] for t in bull_trades) / len(bull_trades)
                print(f"    Bullish (threshold=5): {len(bull_trades)} trades, {wr:.1f}% win, {avg*100:+.1f}% avg, avg score {avg_score:.1f}")
            ...
            if bear_trades:
                ...
                avg_score = sum(t['score'] for t in bear_trades) / len(bear_trades)
                print(f"    Bearish (threshold=4): {len(bear_trades)} trades, {wr:.1f}% win, {avg*100:+.1f}% avg, avg score {avg_score:.1f}")
    ...
    if all_trades:
        all_4plus = [t for t in all_trades if t['score'] >= 4]
        wins = sum(1 for t in all_4plus if t['return'] > 0)
        win_rate = (wins / len(all_4plus)) * 100
        avg_return = sum(t['return'] for t in all_4plus) / len(all_4plus)
        
        print(f"Score 4+ signals: {len(all_4plus)} trades")
        print(f"Win rate: {win_rate:.1f}%")
        print(f"Avg return: {avg_return*100:+.1f}%")
        
        returns = [t['return'] for t in all_4plus]
        if len(returns) > 1:
...
if __name__ == "__main__":
    print("SPY Options Signal Backtest - Dynamic Thresholds")
    print("=" * 70)
